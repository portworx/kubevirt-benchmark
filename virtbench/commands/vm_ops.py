...
@click.option(
    "--nodes", required=True, multiple=True, help="Node names to drain (repeat the flag or pass space-separated)"
)
...
@click.option(
    "--wait-between", type=int, default=None, help="Seconds to wait between nodes (simulates reboot, default: 0)"
)
...
def drain_nodes(ctx, **kwargs):
    ...
    for flag in ("parallel", "ignore_daemonsets", "delete_emptydir", "force", "uncordon_after", "dry_run"):
        if kwargs[flag]:
...
@click.option("--include-master-nodes", is_flag=True, help="Include master/control-plane nodes as rebalance targets")
...
@click.option("--interval", type=int, default=None, help="Seconds between batches (default: 900)")
@click.option("--concurrency", type=int, default=None, help="Max concurrent operations within a batch (default: 50)")
...
@click.option("--end", type=int, required=True, help="End namespace index")
@click.option("--vm-name", required=True, help="VM name in each namespace")
@click.option("--disks", multiple=True, help="Specific disks (e.g., vdb vdc). Auto-detected if omitted.")
...
@click.option("--action", required=True, type=click.Choice(["on", "off"]), help="Whether to power VMs on or off")
@click.option(
    "--node",
    default=None,
    help="Node name. Required for --action off (unless --vm-list-file). " "Optional filter for --action on.",
)
@click.option(
    "--percentage",
    type=int,
...
@click.option("--namespace-prefix", default=None, help="Namespace prefix for range-based --action on")
...
@click.option(
    "--vm-list-file", type=click.Path(exists=True), default=None, help="File of 'namespace/name' lines to act on"
)
@click.option("--concurrency", type=int, default=None, help="Max concurrent operations (default: 50)")
@click.option("--wait-timeout", type=int, default=None, help="Timeout waiting for target phase (default: 300s)")
@click.option("--dry-run", is_flag=True, help="Show what would be done without doing it")
@click.pass_context
