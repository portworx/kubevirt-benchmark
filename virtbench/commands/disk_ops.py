...
@click.option(
    "--operation",
    default="all",
    type=click.Choice(["hotplug", "coldplug", "all"]),
...
@click.option("--parallel-attach", is_flag=True, help="Attach all disks in parallel (default: sequential)")
...
@click.option(
    "--vm-template",
    default="disk-ops-benchmark/vm-template.yaml",
...
@click.option(
    "--ssh-pod", default="ssh-test-pod", help="Persistent SSH helper pod with sshpass (auto-created if missing)"
)
...
@click.option(
    "--vm-timeout", default=600, type=int, help="Timeout for created VMs to reach Running, incl. image import (seconds)"
)
...
def disk_ops(ctx, **kwargs):
    ...
    console.print(
        f"[cyan]Operation:[/cyan] {kwargs['operation']}  "
