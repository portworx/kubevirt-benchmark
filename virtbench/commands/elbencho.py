...
# Deploy parameters
@click.option("--vm-template", default=None, help="Path to VM template YAML (required for deploy and run-all)")
@click.option("--secret-yaml", default=None, help="Path to cloudinit secret YAML file (optional, for deploy/run-all)")
@click.option(
    "--ping-timeout", type=int, default=300, help="Timeout for ping tests in seconds (deploy/run-all, default: 300)"
)
# Workload parameters
@click.option("--iops", type=int, default=0, help="IOPS mode: Total IOPS (split equally between read/write)")
@click.option("--rwmixpct", type=int, default=0, help="rwmixpct mode: Read percentage (0-100)")
...
@click.option("--duration", type=int, default=0, help="Duration in seconds (0 = infinite, REQUIRED for run-all)")
# Results parameters
