...
Examples:
  ...
  %(prog)s --action deploy --start 1 --end 50 --storage-class px-csi \\
    ...
    parser.add_argument(
        "--fio-rw", type=str, default=DEFAULT_FIO_RW, choices=["read", "write", "randread", "randwrite", "randrw", "rw"]
    )
...
def prepare_vm_yaml(
    template_path: str, vm_name: str, storage_class: str, fio_config: Dict, vm_password: str, logger
) -> str:
...
def deploy_vm(namespace: str, vm_yaml: str, logger) -> bool:
    ...
    try:
        subprocess.run(
            ["kubectl", "apply", "-f", "-", "-n", namespace], input=vm_yaml, capture_output=True, text=True, check=True
        )
...
def get_vm_and_vmi_status(namespace: str, vm_name: str) -> Tuple[str, str]:
    ...
    try:
        result = subprocess.run(
            ["kubectl", "get", "vm", "-n", namespace, vm_name, "-o", "jsonpath={.status.printableStatus}"],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    try:
        result = subprocess.run(
            ["kubectl", "get", "vmi", "-n", namespace, vm_name, "-o", "jsonpath={.status.phase}"],
            capture_output=True,
            text=True,
            check=False,
        )
...
def run_ssh_command(
    vm_ip: str, command: str, ssh_pod: str, ssh_pod_ns: str, vm_user: str, vm_password: str, timeout: int = 60
) -> Optional[str]:
    """Run SSH command on VM via the shared helper. Returns stdout, or None on exec failure."""
    try:
        _, stdout, _ = ssh_exec_command(vm_ip, command, ssh_pod, ssh_pod_ns, vm_user, vm_password, timeout=timeout)
        return stdout
    except (subprocess.TimeoutExpired, subprocess.CalledProcessError):
        return None


def wait_for_fio_complete(namespace: str, vm_name: str, ssh_config: Dict, timeout: int, logger) -> bool:
    ...
    while time.time() - start < timeout:
        output = run_ssh_command(
            vm_ip,
            "cat /tmp/fio_complete 2>/dev/null",
            ssh_config["pod"],
            ssh_config["pod_ns"],
            ssh_config["user"],
            ssh_config["password"],
...
def collect_fio_results(
    namespace: str, vm_name: str, ssh_config: Dict, output_dir: str, logger, max_retries: int = 8, retry_delay: int = 20
) -> Optional[Dict]:
...
def aggregate_results(results: List[Dict], fio_config: Dict, total_duration: float) -> Dict:
    ...
    def calc_stats(key):
        ...
        return {"avg": round(sum(values) / len(values), 2), "max": round(max(values), 2), "min": round(min(values), 2)}
...
def action_deploy(args, namespaces, fio_config, logger):
    ...
    vm_yaml = prepare_vm_yaml(template_path, args.vm_name, args.storage_class, fio_config, args.vm_password, logger)
    ...
    print("Use --action status to check progress.")
    print("Use --action gather-results to collect results when complete.")
...
def check_fio_status_in_vm(vm_ip: str, ssh_config: Dict, logger) -> str:
    ...
    fio_running = run_ssh_command(
        vm_ip,
        'pgrep -x fio >/dev/null 2>&1 && echo "YES" || echo "NO"',
        ssh_config["pod"],
        ssh_config["pod_ns"],
        ssh_config["user"],
        ssh_config["password"],
    ...
    fio_results = run_ssh_command(
        vm_ip,
        'test -f /tmp/fio_results.json && echo "YES" || echo "NO"',
        ssh_config["pod"],
        ssh_config["pod_ns"],
        ssh_config["user"],
        ssh_config["password"],
...
def action_status(args, namespaces, ssh_config, logger):
    ...
    print("-" * 70)
    print(
        f"Summary: {summary['completed']} completed, {summary['running']} running, "
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.collect_concurrency)) as executor:
        futures = {
            executor.submit(
                ...
                output_dir,
                logger,
                args.collect_retries,
                args.collect_retry_delay,
            ): ns
            for ns in namespaces
        }
        for future in as_completed(futures):
...
def action_run_all(args, namespaces, fio_config, ssh_config, logger):
    ...
    print(f"Storage Class: {args.storage_class}")
    print(
        f"FIO Config: {fio_config['rw']} | bs={fio_config['bs']} | iodepth={fio_config['iodepth']} | "
    ...
    vm_yaml = prepare_vm_yaml(
        template_path, args.vm_name, args.storage_class, fio_config, ssh_config["password"], logger
    )
    ...
    print("[3/4] Waiting for VMs to boot and FIO to complete (polling every 30s)...")
    fio_timeout = fio_config["runtime"] + 600  # FIO runtime + boot time + buffer

    completed = []
    with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
        futures = {
            executor.submit(wait_for_fio_complete, ns, args.vm_name, ssh_config, fio_timeout, logger): ns
            for ns in namespaces
        }
        for future in as_completed(futures):
            ns = futures[future]
...
def main():
    ...
    ssh_config = {"pod": args.ssh_pod, "pod_ns": args.ssh_pod_ns, "user": args.vm_user, "password": args.vm_password}
