...
from utils.common import run_kubectl_command, setup_logging
...
class ClusterValidator:
    ...
    def check_kubevirt_installed(self) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "kubevirt", "-A", "-o", "json"], check=False, logger=self.logger
        )
        ...
        if returncode == 0:
            ...
            if len(items) > 0:
                ...
                if phase == "Deployed":
                    ...
                    return (
                        False,
                        f"KubeVirt '{name}' found in namespace '{namespace}' but phase is '{phase}' (expected: Deployed)",
                    )
            else:
    ...
    def _check_kubevirt_components(self, namespace: str) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "deployment", "-n", namespace, "-o", "json"], check=False, logger=self.logger
        )
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "daemonset", "virt-handler", "-n", namespace, "-o", "json"], check=False, logger=self.logger
        )
        ...
        if returncode == 0:
            ...
            return (
                True,
                f"OpenShift Virtualization is deployed in '{namespace}' (virt-api, virt-controller, virt-operator, virt-handler ready)",
            )
        else:
            self.warnings += 1
    ...
    def check_storage_class(self, storage_class_name: str) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "storageclass", storage_class_name, "-o", "json"], check=False, logger=self.logger
        )
        if returncode != 0:
    ...
    def check_worker_nodes(self, min_nodes: int = 1) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "nodes", "-l", "node-role.kubernetes.io/worker", "-o", "json"], check=False, logger=self.logger
        )
        if returncode != 0:
    ...
    def check_node_resources(self) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(["top", "nodes"], check=False, logger=self.logger)
        if returncode != 0:
            self.warnings += 1
    ...
    def check_datasource(self, datasource_name: str, namespace: str) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "datasource", datasource_name, "-n", namespace, "-o", "json"], check=False, logger=self.logger
        )
        if returncode != 0:
            self.warnings += 1
    ...
    def check_ssh_pod(self, pod_name: str, namespace: str) -> Tuple[bool, str]:
        ...
        returncode, stdout, stderr = run_kubectl_command(
            ["get", "pod", pod_name, "-n", namespace, "-o", "json"], check=False, logger=self.logger
        )
        if returncode != 0:
            self.warnings += 1
    ...
    def check_permissions(self) -> Tuple[bool, str]:
        ...
        for verb, resource in permissions:
            returncode, stdout, stderr = run_kubectl_command(
                ["auth", "can-i", verb, resource, "--all-namespaces"], check=False, logger=self.logger
            )
...
Examples:
  ...
  %(prog)s --storage-class YOUR-STORAGE-CLASS --datasource rhel9 --datasource-namespace openshift-virtualization-os-images
    ...
    parser.add_argument("--storage-class", type=str, help="Storage class name to validate")
    parser.add_argument("--datasource", type=str, default="rhel9", help="DataSource name to validate (default: rhel9)")
    parser.add_argument(
    ...
    parser.add_argument("--ssh-pod", type=str, default="ssh-test-pod", help="SSH test pod name (default: ssh-test-pod)")
    parser.add_argument(
        "--ssh-pod-namespace", type=str, default="default", help="SSH test pod namespace (default: default)"
    )
    parser.add_argument(
        "--min-worker-nodes", type=int, default=1, help="Minimum number of worker nodes required (default: 1)"
    )
...
def main():
    ...
    if not args.quick and (args.all or args.datasource):
        validator.run_check(
            f"DataSource '{args.datasource}'", validator.check_datasource, args.datasource, args.datasource_namespace
        )

    if not args.quick and (args.all or args.ssh_pod):
        validator.run_check(
            f"SSH test pod '{args.ssh_pod}'", validator.check_ssh_pod, args.ssh_pod, args.ssh_pod_namespace
        )
